#!/usr/bin/env python3
"""
Systematic Augmentation Experiments for Research Paper
Comprehensive ablation study of data augmentation techniques
"""
...
from augmentation_config import AugmentationExperiments, AugmentationConfig
...
def run_single_experiment(exp_name: str, exp_config: dict, base_cmd: list) -> bool:
    ...
    # Create augmentation config file for this experiment
    aug_config_path = f"temp_aug_config_{exp_name}.json"
    with open(aug_config_path, 'w') as f:
        json.dump(exp_config['augmentations'], f, indent=2)
    
    # Prepare command
    cmd = base_cmd + [
        '--augmentation_config', aug_config_path,
        '--experiment_name', f"aug_{exp_name}"
    ]
    ...
    try:
        ...
        # Clean up temp config file (Python 3.7 compatible)
        try:
            Path(aug_config_path).unlink()
...
def run_experiment_group(group_name: str, base_cmd: list) -> dict:
    ...
    experiments = AugmentationExperiments.get_experiments_by_group(group_name)
    ...
    table.add_column("Augmentations", style="yellow")
    
    for exp_name, exp_config in experiments.items():
        aug_summary = ", ".join([f"{k}={v}" for k, v in exp_config['augmentations'].items()])
        if not aug_summary:
            aug_summary = "None (Baseline)"
        table.add_row(exp_name, exp_config['name'], aug_summary)
...
def create_results_summary(all_results: dict, save_dir: Path):
    ...
    summary_path = save_dir / "augmentation_experiments_summary.json"
...
def print_final_summary(all_results: dict):
    ...
    console.print("\n[bold magenta]═══ FINAL AUGMENTATION EXPERIMENTS SUMMARY ═══[/bold magenta]")
...
def main():
    parser = argparse.ArgumentParser(description='Run Systematic Augmentation Experiments')
    parser.add_argument('--group', type=str, choices=AugmentationExperiments.get_experiment_groups(),
    ...
    console.print("[bold magenta]Systematic Augmentation Experiments[/bold magenta]")
    ...
    console.print("• Objective: Systematic augmentation ablation study")
    ...
    results_dir = Path("outputs") / "augmentation_experiments" 
    ...
    if args.group:
        ...
        experiment_groups = AugmentationExperiments.get_experiment_groups()
