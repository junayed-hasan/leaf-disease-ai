#!/usr/bin/env python3
"""
Evaluate Baseline Student Model on Test Datasets
Tests the baseline trained student model on test sets of tomatoVillage and plantVillage datasets
"""
...
class BaselineStudentTestEvaluator:
    """Baseline student model evaluator for test datasets"""
    ...
    def __init__(self, model_name, model_path, num_classes, device):
        ...
        console.print(f"Loading baseline student model {model_name} from {model_path}")
        self.model = self._load_model()
        self.model.eval()  # Set to evaluation mode
        
        console.print(f"[green]Successfully loaded baseline student model: {model_name}[/green]")
    ...
    def predict(self, data_loader):
        """Get baseline student model predictions"""
        all_preds = []
        all_targets = []
        all_probs = []
        all_samples = []
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets, samples) in enumerate(data_loader):
                ...
                # Get predictions from baseline student model
...
def get_baseline_student_model():
    """Get the baseline student model path"""
    
    # Use the specific baseline model path provided by user
    baseline_path = Path("outputs/shufflenet_v2/combined_augment_shuffle_baseline_20250525015352/best_model.pth")
    
    if baseline_path.exists():
        console.print(f"Found baseline student model at: {baseline_path}")
        return "shufflenet_v2", baseline_path
    
    # Fallback: search for baseline student model
    console.print("[yellow]Specific baseline model not found, searching for baseline student model[/yellow]")
    
    student_base_dir = Path("outputs") / "shufflenet_v2"
    if student_base_dir.exists():
        for run_dir in student_base_dir.iterdir():
            if run_dir.is_dir() and "baseline" in run_dir.name:
                model_file = run_dir / "best_model.pth"
                
                if model_file.exists():
                    console.print(f"Found baseline student model at: {model_file}")
                    return "shufflenet_v2", model_file
    
    console.print("[red]No baseline student model found[/red]")
    return None, None
...
def evaluate_baseline_student_on_test_dataset(student_preds, student_targets, student_probs, 
                                             all_samples, class_mapping, save_dir, dataset_name, student_model_name):
    """Evaluate baseline student model performance on test dataset"""
    ...
    performance = {
        ...
        "student_model": f"{student_model_name}_baseline",
    ...
    # Print results
    console.print(f"\n[bold]Baseline Student Model ({student_model_name}) Performance on {dataset_name} Test Dataset:[/bold]")
    ...
    with open(save_dir / f"{dataset_name}_baseline_student_classification_report.json", "w") as f:
    ...
    plt.title(f'Baseline Student Model ({student_model_name}) Performance on {dataset_name} Test Dataset - Confusion Matrix')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(save_dir / f"{dataset_name}_baseline_student_confusion_matrix.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open(save_dir / f"{dataset_name}_baseline_student_misclassifications.json", "w") as f:
    ...
    with open(save_dir / f"{dataset_name}_baseline_student_performance_metrics.json", "w") as f:
        json.dump(performance, f, indent=4)
    
    # Log to wandb
    wandb.log({
        f"{dataset_name}_baseline_student_accuracy": accuracy * 100,
        f"{dataset_name}_baseline_student_f1_macro": f1_macro * 100,
        f"{dataset_name}_baseline_student_f1_weighted": f1_weighted * 100,
        f"{dataset_name}_baseline_student_precision_macro": precision_macro * 100,
        f"{dataset_name}_baseline_student_recall_macro": recall_macro * 100,
        f"{dataset_name}_baseline_student_total_samples": len(student_targets),
        f"{dataset_name}_baseline_student_misclassification_rate": len(misclassified)/len(student_targets)*100,
        f"baseline_student_model_parameters": num_params
    })
    
    return performance

def evaluate_single_dataset(dataset_name: str, student_evaluator, class_mapping, save_dir, student_model_name):
    """Evaluate baseline student model on a single test dataset"""
    ...
    console.print(f"\n[bold]Getting Baseline Student Model Predictions on {dataset_name}[/bold]")
    ...
    console.print(f"\n[bold]Evaluating Baseline Student Model Performance on {dataset_name}[/bold]")
    performance = evaluate_baseline_student_on_test_dataset(
...
def main():
    """Evaluate the baseline student model on test datasets"""
    
    console.print("[bold]Evaluating Baseline Student Model on Test Datasets[/bold]")
    ...
    # Find the trained baseline student model
    console.print("\n[bold]Step 1: Finding Baseline Student Model[/bold]")
    student_model_name, baseline_model_path = get_baseline_student_model()
    
    if student_model_name is None or baseline_model_path is None:
        console.print("[red]No baseline student model found.[/red]")
        return False
    
    console.print(f"Baseline Student Model: {student_model_name}")
    console.print(f"Model Path: {baseline_model_path}")
    
    # Create run directory
    timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
    run_dir = Path("outputs") / "test_evaluation" / "baseline_student" / f"baseline_student_on_test_datasets_{timestamp}"
    ...
    # Create config
    config = {
        "evaluation_type": "baseline_student_on_test_datasets",
    ...
    # Initialize wandb
    wandb_name = f"baseline_student_test_eval_{timestamp}"
    run = wandb.init(
    ...
    try:
        # Create baseline student model evaluator
        console.print("\n[bold]Step 3: Creating Baseline Student Model Evaluator[/bold]")
        student_evaluator = BaselineStudentTestEvaluator(
        ...
        # Print summary
        console.print(f"\n[bold]Baseline Student Model Test Evaluation Summary:[/bold]")
        ...
        console.print(f"\n[green]Baseline student model evaluation on test datasets completed successfully![/green]")
        console.print(f"Results saved to: {run_dir}")
        
        return True
        
    except Exception as e:
        console.print(f"[red]✗ Baseline student model test evaluation failed with exception: {e}[/red]")
        raise e
    finally:
        # Close wandb run
        wandb.finish()
