#!/usr/bin/env python3
"""
Training Script for Hyperparameter Experiments
Based on original train.py but with hyperparameter configuration support
"""
...
from trainer_hp import Trainer
...
def main():
    parser = argparse.ArgumentParser(description='Train model with hyperparameter experiments')
    ...
    if args.experiment_name is None:
        hp_summary = f"lr{args.learning_rate}_opt{args.optimizer}_sched{args.scheduler}_wd{args.weight_decay}"
        args.experiment_name = f"{args.model}_hp_{hp_summary}"
    ...
    hp_description = f"lr{args.learning_rate}_{args.optimizer}_{args.scheduler}"
    wandb_name = f"{args.dataset}_{args.model}_hp_{hp_description}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    
    # Initialize wandb
    run = wandb.init(
        project="tomato-disease-hyperparameters",
        name=wandb_name,
        config=config
    )
    ...
    optimizer = HyperparameterConfig.get_optimizer(
        args.optimizer, model.parameters(), args.learning_rate, args.weight_decay
    )
    scheduler = HyperparameterConfig.get_scheduler(args.scheduler, optimizer, args.epochs)
    ...
    console.print(f"  Weight Decay: {args.weight_decay}")
    ...
    # Copy source files
    for file in ["train_hyperparameter.py", "trainer_hp.py", "config.py", "dataset.py", "hyperparameter_config.py"]:
        if Path(file).exists():
